...
async function main(): Promise<void> {
  ...
  child.on("error", (err) =>
    console.error("[smoke] adapter process error:", err),
  );
  child.on("exit", (code, signal) =>
    console.log(
      `[smoke] adapter process exited (code=${code}, signal=${signal})`,
    ),
  );
  child.stderr.on("data", (chunk: Buffer) =>
    process.stderr.write(`[lldb-dap stderr] ${chunk}`),
  );
  ...
  const launchArgs = lldbDapAdapter.buildLaunchArgs({
    program: PROGRAM,
    cwd: FIXTURE_DIR,
  });
