...
function waitForPhase(
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error(`timed out waiting for phase "${phase}" (${label})`)), ms);
    ...
    const unsubscribe = session.onSnapshot((snapshot) => {
      if (snapshot.phase === phase) {
        ...
        clearTimeout(timer);
        unsubscribe();
        reject(new Error(`session entered error phase while waiting for "${phase}": ${snapshot.errorMessage}`));
      }
    });
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const unsubscribe = session.onSnapshot((snapshot) => {
      if (predicate(snapshot)) {
        ...
        clearTimeout(timer);
        unsubscribe();
        reject(new Error(`session entered error phase while waiting for "${label}": ${snapshot.errorMessage}`));
      }
    });
  });
}
...
async function main(): Promise<void> {
  const session = new DebugSession(
    { adapterId: lldbDapAdapter.id, programPath: PROGRAM, sourcePath: SOURCE, cwd: FIXTURE_DIR },
    lldbDapAdapter
  );
  ...
  const stopped = await waitForPhase(session, 'stopped', 'first breakpoint hit');
  console.log('[session-smoke] hit breakpoint, stack:', stopped.stack.map((f) => `${f.name}@${f.line}`));
  assert.ok(stopped.stack.length > 0, 'expected a non-empty call stack');
  assert.ok(stopped.stack[0]?.name.startsWith('loop_demo::main'), 'expected top frame to be loop_demo::main');
  ...
  assert.equal(watch.value, '0', 'expected watch "total * 2" to evaluate to 0 at first stop');
  console.log('[session-smoke] watch evaluated:', watch);

  const stepPromise = waitForNextSnapshot(session, (s) => s.phase === 'stopped', 'after stepOver');
  await session.stepOver();
  const afterStep = await stepPromise;
  const localsScope2 = afterStep.scopes.find((s) => s.name === 'Locals');
  const localVars2 = afterStep.variablesByRef[localsScope2!.variablesReference] ?? [];
  const byName2 = Object.fromEntries(localVars2.map((v) => [v.name, v.value]));
  console.log('[session-smoke] locals after stepOver:', byName2);
  assert.equal(byName2['total'], '2', 'expected total=2 after stepping over the accumulation line');

  const watchAfterStep = afterStep.watches[0];
  assert.equal(watchAfterStep?.value, '4', 'expected watch to auto-reevaluate to 4 (total=2 * 2)');
  console.log('[session-smoke] watch auto-reevaluated after step:', watchAfterStep);
  ...
  assert.equal(afterRemove.watches.length, 0, 'expected watch list to be empty after removeWatch');
