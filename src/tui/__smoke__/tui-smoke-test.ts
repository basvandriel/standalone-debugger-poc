...
async function main(): Promise<void> {
  ...
  await waitFor(() => useDbgStore.getState().snapshot?.phase === 'configuring', 'configuring phase');
  await sleep(200); // let a render flush so lastFrame() reflects it
  console.log('[tui-smoke] frame after reaching configuring:\n' + lastFrame());
  assert.ok(lastFrame()?.includes('READY'), 'expected READY phase badge in configuring state');
  assert.ok(lastFrame()?.includes('main.rs'), 'expected source file name visible');
  ...
  stdin.write('c');
  await waitFor(() => useDbgStore.getState().snapshot?.phase === 'stopped', 'stopped at breakpoint');
  await sleep(300);
  ...
  const localVars = localsScope ? stoppedSnapshot?.variablesByRef[localsScope.variablesReference] : undefined;
  ...
  stdin.write(':');
  await sleep(50);
  assert.equal(useUiStore.getState().commandBarOpen, true, 'expected command bar to open');
  for (const ch of 'watch total * 2') stdin.write(ch);
  stdin.write('\r');
  await waitFor(() => (useDbgStore.getState().snapshot?.watches.length ?? 0) > 0, 'watch added');
  const watch = useDbgStore.getState().snapshot?.watches[0];
  assert.equal(watch?.expression, 'total * 2');
  assert.equal(watch?.value, '0', 'expected watch to evaluate to 0 at first stop');
  ...
  stdin.write('c');
  await waitFor(() => useDbgStore.getState().snapshot?.phase === 'terminated', 'program terminated', 15_000);
  await sleep(300);
  ...
  assert.equal(finalSnapshot?.stack.length, 0, 'expected stack cleared after termination');
  assert.equal(finalSnapshot?.watches[0]?.error, 'not running', 'expected watch marked not running');
  ...
  stdin.write('c');
  await waitFor(() => useDbgStore.getState().snapshot?.phase === 'configuring', 'configuring after restart');
  ...
  const bpAfterRestart = useDbgStore.getState().snapshot?.breakpoints[SOURCE]?.[0];
  assert.equal(bpAfterRestart?.verified, true, 'expected breakpoint to be verified on the restarted adapter');

  stdin.write('c');
  await waitFor(() => useDbgStore.getState().snapshot?.phase === 'stopped', 'stopped at breakpoint after restart');
  const stoppedAfterRestart = useDbgStore.getState().snapshot;
  assert.equal(stoppedAfterRestart?.stack[0]?.line, 7, 'expected restarted session to hit the same breakpoint');
  ...
  unmount();
  console.log('[tui-smoke] SUCCESS: TUI rendered and interactive against real lldb-dap session end to end.');
  process.exit(0);
}
