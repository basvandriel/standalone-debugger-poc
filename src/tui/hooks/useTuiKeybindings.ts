 ...
 */
export function useTuiKeybindings(session: DebugSession, snapshot: SessionSnapshot | undefined, exit: () => void): void {
  useInput((input, key) => {
    ...
    if (input === TUI_KEYS.startContinue) {
      ...
      else if (snapshot.phase === 'terminated' || snapshot.phase === 'error') void session.restart();
      return;
    }
    ...
    if (scrollDelta !== 0) {
      if (ui.focusedPanel === 'source') {
        ...
        ui.setCursorLine(Math.min(Math.max(1, ui.cursorLine + scrollDelta), Math.max(1, ui.sourceLines.length)));
        return;
      }
      if (ui.focusedPanel === 'stack') {
        const idx = snapshot.stack.findIndex((f) => f.id === snapshot.selectedFrameId);
        const next = snapshot.stack[Math.min(snapshot.stack.length - 1, Math.max(0, (idx === -1 ? 0 : idx) + scrollDelta))];
        if (next) void session.selectFrame(next.id);
        return;
      }
      if (ui.focusedPanel === 'variables') {
        const maxIdx = Math.max(0, flattenScopes(snapshot.scopes, snapshot.variablesByRef, ui.expandedRefs).length - 1);
        ui.setSelectedVariableIndex(Math.min(Math.max(0, ui.selectedVariableIndex + scrollDelta), maxIdx));
        return;
      }
      if (ui.focusedPanel === 'watch') {
        const maxIdx = Math.max(0, snapshot.watches.length - 1);
        ui.setSelectedWatchIndex(Math.min(Math.max(0, ui.selectedWatchIndex + scrollDelta), maxIdx));
        return;
      }
    }

    const isDown = key.downArrow || input === TUI_KEYS.moveDown || input === '\x1b[B';
    ...
    if (ui.focusedPanel === 'source') {
      if (isDown) ui.setCursorLine(Math.min(ui.cursorLine + 1, Math.max(1, ui.sourceLines.length)));
    ...
    if (ui.focusedPanel === 'stack') {
      ...
      if (isDown && snapshot.stack.length > 0) {
        const next = snapshot.stack[Math.min(snapshot.stack.length - 1, (idx === -1 ? 0 : idx) + 1)];
        if (next) void session.selectFrame(next.id);
    ...
    if (ui.focusedPanel === 'variables') {
      const rows = flattenScopes(snapshot.scopes, snapshot.variablesByRef, ui.expandedRefs);
      ...
      if (isDown) {
        ...
        if (row?.expandable) {
          if (!row.expanded) void session.expandVariable(row.variablesReference);
    ...
    if (ui.focusedPanel === 'watch') {
      if (isDown && snapshot.watches.length > 0) {
        ui.setSelectedWatchIndex(Math.min(snapshot.watches.length - 1, ui.selectedWatchIndex + 1));
      } else if (isUp && snapshot.watches.length > 0) {
        ui.setSelectedWatchIndex(Math.max(0, ui.selectedWatchIndex - 1));
      } else if ((key.delete || key.backspace || input === TUI_KEYS.removeWatch) && snapshot.watches.length > 0) {
    ...
    if (ui.focusedPanel === 'console') {
      if (isLeft || isRight) ui.setOutputTab(ui.outputTab === 'program' ? 'dap' : 'program');
    }
  });
}
